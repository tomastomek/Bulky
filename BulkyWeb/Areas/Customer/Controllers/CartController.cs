...
namespace BulkyWeb.Areas.Customer.Controllers
{
    ...
    public class CartController : Controller
    {
        ...
        public IActionResult Index()
        {
            ...
            {
                cart.Product.ProductImages = productImages.Where(c => c.ProductId == cart.ProductId).ToList();
        ...
        [HttpPost]
        [ActionName("Summary")]
        // because we used bind model on ShoppingCartVM, it will be automatically populated
        // when form is posted
        public IActionResult SummaryPOST()
        {
            ...
            // GetValueOrDefault - because company id can be null
            ...
            if (applicationUser.CompanyId.GetValueOrDefault() == 0)
            {
                var domain = Request.Scheme + "://" + Request.Host.Value + "/"; // https://localhost:7071/";
                // stripe logic - we need to capture payment
                ...
                // payment intent id is populated once the payment is successful, otherwise it's null
                // here we are just creating the session, so the intent id will be null
                _unitOfWork.OrderHeader.UpdateStripePaymentID(ShoppingCartVM.OrderHeader.Id, session.Id, session.PaymentIntentId);
                _unitOfWork.Save();

                // redirect to stripe to process payment
                Response.Headers.Add("Location", session.Url);
                // we're redirecting to new location
